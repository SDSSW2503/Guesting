...
public class RegistService {
	...
	public Regist requestGuesting(Integer memberId, Integer otherTeamId) {
		Member member = memberRepository.getByMemberId(memberId).orElseThrow(()->new AuthException("잘못된 접근입니다. 다시 로그인 해주세요."));
		Team curTeam = member.getTeam();
		
		if (Objects.isNull(curTeam)) throw new BusinessException("팀 가입 후 이용해주세요.");
		...
		if(registRepository.getMatchedTeam(curTeam.getTeamId()).isPresent()) {
			throw new BusinessException("이미 팀의 매칭이 완료된 상태입니다.");
		}
		//이미 다른 곳에 요청을 보낸 상태인지
		if(!registRepository.getSentRegists(curTeam.getTeamId()).isEmpty()) {
			throw new BusinessException("이미 보낸 매칭 신청이 존재합니다.");
		}
		//상대 팀이 이미 매칭이 된 상태인지
		if(registRepository.getMatchedTeam(otherTeamId).isPresent()) {
			throw new BusinessException("이미 매칭이 완료된 팀입니다.");
		}
  
  		//fetch all houses, and select one at random
		List<House> houseList = houseRepository.findAll(); 
		if (houseList.isEmpty()) throw new BusinessException("empty house DB");
	...
	public List<RegistRes> viewSentRegistRecords(Integer memberId) {
		//1) session 에서 나의 팀 아이디를 꺼낸다.
		Member member = memberRepository.getByMemberId(memberId).orElseThrow(()->new AuthException("잘못된 접근입니다. 다시 로그인 해주세요."));
		if (Objects.isNull(member)) throw new BusinessException("다시 로그인 해주세요.");
		Team curTeam = member.getTeam();
	...
	@Transactional
	public void acceptRegist(Integer registId, Integer memberId) {
		Member member = memberRepository.getByMemberId(memberId).orElseThrow(()->new AuthException("잘못된 접근입니다. 다시 로그인 해주세요."));
		Team curTeam = member.getTeam();
		...
		Regist regist = registRepository.findById(registId)
				.orElseThrow(()->new BusinessException("찾을 수 없는 요청입니다."));
		
		if (!Objects.equals(regist.getReceiveTeam().getTeamId(), curTeam.getTeamId()))
			throw new BusinessException("자신이 소속된 팀의 게스팅만 수락할 수 있습니다..");
		//이미 거절된 요청이면 에러 반환
		if(regist.getStatus().equals(Status.DECLINED)) {
			throw new BusinessException("이미 거절된 요청을 수락할 수 없습니다.");
		}
	...
	@Transactional
	public void declineRegist(Integer registId, Integer memberId) {
		Member member = memberRepository.getByMemberId(memberId).orElseThrow(()->new AuthException("잘못된 접근입니다. 다시 로그인 해주세요."));
		if (Objects.isNull(member)) throw new AuthException("다시 로그인 해주세요.");
		Team curTeam = member.getTeam();
		...
		if (!Objects.equals(regist.getReceiveTeam().getTeamId(), curTeam.getTeamId()))
			throw new BusinessException("자신이 소속된 팀의 게스팅만 거절할 수 있습니다..");
	 ...
	 //5. 성사된 매칭을 조회한다.
	public RegistRes getMyMatching (Integer memberId) {
		Member member = memberRepository.getByMemberId(memberId).orElseThrow(()->new AuthException("잘못된 접근입니다. 다시 로그인 해주세요."));
		if (Objects.isNull(member)) throw new AuthException("다시 로그인 해주세요.");
		Team curTeam = member.getTeam();
		...
		Regist regist = registRepository.getMatchedTeam(curTeam.getTeamId())
				.orElseThrow(() -> new BusinessException("성사된 매칭이 존재하지 않습니다."));
